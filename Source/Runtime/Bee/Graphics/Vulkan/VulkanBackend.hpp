...
struct VulkanShader // NOLINT
{
    static constexpr u32 max_name = 256;
...
struct VulkanBuffer
{
    ...
    VulkanBuffer(const BufferType new_type, const u32 new_size)
        : type(new_type),
          size(new_size)
    {}
};
...
struct VulkanCommandPool
{
    struct PerQueuePool
    {
        ...
        DynamicArray<GpuCommandBuffer*> command_buffers;
    };
...
struct VulkanFramebuffer // NOLINT
{
...
struct VulkanDevice
{
    ...
    using render_pass_table_t   = HandleTable<BEE_GPU_MAX_RENDER_PASSES, RenderPassHandle, VulkanRenderPass>;
    using shader_table_t        = HandleTable<BEE_GPU_MAX_SHADERS, ShaderHandle, VulkanShader>;
    using buffer_table_t        = HandleTable<BEE_GPU_MAX_BUFFERS, BufferHandle, VulkanBuffer>;
    ...
    using command_pool_table_t  = HandleTable<BEE_GPU_MAX_COMMAND_POOLS, CommandPoolHandle, VulkanCommandPool>;
    using fence_table_t         = HandleTable<BEE_GPU_MAX_FENCES, FenceHandle, VkFence>;
    using pipeline_table_t      = HandleTable<BEE_GPU_MAX_PIPELINE_STATES, PipelineStateHandle, VulkanPipeline>;
