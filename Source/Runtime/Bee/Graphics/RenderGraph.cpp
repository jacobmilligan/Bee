...
RenderGraphBuilder& RenderGraphBuilder::write_color(const RenderGraphResource& texture, const LoadOp load, const StoreOp store)
{
...
RenderGraphBuilder& RenderGraphBuilder::write_depth(const RenderGraphResource& texture, const PixelFormat depth_format, const LoadOp load, const StoreOp store)
{
    BEE_ASSERT(texture.is(RenderGraphResourceType::texture) || texture.is(RenderGraphResourceType::imported_texture));
...
RenderPassHandle RenderGraph::obtain_physical_pass(const DeviceHandle& device, PhysicalPassPool* pool, const RenderPassCreateInfo& create_info)
{
    const auto hash = get_hash(create_info);
    const auto index = container_index_of(pool->hashes, [&](const u32 stored_hash) { return stored_hash == hash; });
