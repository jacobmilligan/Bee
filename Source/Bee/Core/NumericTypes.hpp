...
inline bool operator==(const u128& lhs, const u128& rhs)
{
 ...
 * # sign_cast
 *
 * casts between signed and unsigned fundamental
 * types and if assertions are enabled will do a
 * fatal assert if the sign change causes the
 * result to overflow or underflow
 *
 **************************************************
 */
...
template <typename ResultType, typename ValueType>
constexpr ResultType sign_cast(const ValueType& value)
{
    return static_cast<ResultType>(value);
}
