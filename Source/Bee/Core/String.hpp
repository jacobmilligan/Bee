...
BEE_API i32 system_snprintf(char* buffer, size_t buffer_size, const char* format, va_list args);
 ...
 *   * if lhs == rhs then the return value is == 0
 */
BEE_API i32 compare_n(const char* lhs, i32 lhs_compare_count, const char* rhs, i32 rhs_compare_count);

BEE_API i32 compare_n(const char* lhs, const char* rhs, i32 max_compare_count);

BEE_API i32 compare_n(const String& lhs, const char* rhs, i32 max_compare_count);

BEE_API i32 compare_n(const StringView& lhs, const char* rhs, i32 max_compare_count);

BEE_API i32 compare(const String& lhs, const String& rhs);

BEE_API i32 compare(const String& lhs, const char* rhs);

BEE_API i32 compare(const String& lhs, const StringView& rhs);

BEE_API i32 compare(const StringView& lhs, const StringView& rhs);

BEE_API i32 compare(const StringView& lhs, const char* rhs);
 ...
 */
BEE_API i32 length(const char* string);


/**
 * `string_copy` - c-string copy function - mostly a wrapper around strncpy
 */
BEE_API void copy(char* dst, i32 dst_size, const char* src, i32 src_count);

BEE_API void copy(char* dst, i32 dst_size, const StringView& src);


/**
 * `string_format` - format a string with printf-like format characters (similar to snprintf)
 */
BEE_API String format(Allocator* allocator, const char* format, ...) BEE_PRINTFLIKE(2, 3);

BEE_API String format(const char* format, ...) BEE_PRINTFLIKE(1, 2);

BEE_API void format(char* buffer, i32 buffer_size, const char* format, ...) BEE_PRINTFLIKE(3, 4);


/**
 * `last_index_of` - finds the last occurence of a character or substring in the `src` string. Returns -1 if not found
 */
BEE_API i32 last_index_of_n(const String& src, const char* substring, i32 substring_size);

BEE_API i32 last_index_of_n(const StringView& src, const char* substring, i32 substring_size);


BEE_API i32 last_index_of(const String& src, char character);

BEE_API i32 last_index_of(const String& src, const char* substring);

BEE_API i32 last_index_of(const String& src, const String& substring);

BEE_API i32 last_index_of(const StringView& src, char character);

BEE_API i32 last_index_of(const StringView& src, const char* substring);

BEE_API i32 last_index_of(const StringView& src, const StringView& substring);

BEE_API i32 last_index_of(const StringView& src, const String& substring);


/**
 * `first_index_of` - finds the first occurence of a character or substring in the `src` string. Returns -1 if not found
 */
BEE_API i32 first_index_of_n(const String& src, const char* substring, i32 substring_size);

BEE_API i32 first_index_of_n(const StringView& src, const char* substring, i32 substring_size);


BEE_API i32 first_index_of(const String& src, char character);

BEE_API i32 first_index_of(const String& src, const char* substring);

BEE_API i32 first_index_of(const String& src, const String& substring);

BEE_API i32 first_index_of(const StringView& src, char character);

BEE_API i32 first_index_of(const StringView& src, const char* substring);

BEE_API i32 first_index_of(const StringView& src, const StringView& substring);

BEE_API i32 first_index_of(const StringView& src, const String& substring);

/**
 * `replace` - replaces all occurrences of `old_char` with `new_char` in string `src`
 */
BEE_API String& replace(String* src, char old_char, char new_char);

/**
 * `replace` - replaces all occurrences of `old_string` with `new_string` in string `src`
 */
BEE_API String& replace(String* src, const char* old_string, const char* new_string);

BEE_API String& replace(String* src, const String& old_string, const char* new_string);

BEE_API String& replace(String* src, const char* old_string, const String& new_string);

/**
 * `replace_n` - replaces the first n occurrences of `old_char` with `new_char` in string `src`
 */
BEE_API String& replace_n(String* dst, char old_char, char new_char, i32 count);

/**
 * `replace_n` - replaces the first n occurrences of `old_string` with `new_string` in string `src`
 */
BEE_API String& replace_n(String* dst, const char* old_string, const char* new_string, i32 count);

BEE_API String& replace_n(String* dst, const String& old_string, const char* new_string, i32 count);

BEE_API String& replace_n(String* dst, const char* old_string, const String& new_string, i32 count);

/**
 * `replace_range` - replaces all characters in the range `index` to `index` + `size` with `new_char`
 */
BEE_API String& replace_range(String* src, i32 index, i32 size, char new_char);

/**
 * `replace_range` - replaces all characters in the range `index` to `index` + `size` with `new_string`
 */
BEE_API String& replace_range(String* src, i32 index, i32 size, const char* new_string, i32 new_string_size);

BEE_API String& replace_range(String* src, i32 index, i32 size, const char* new_string);

BEE_API String& replace_range(String* src, i32 index, i32 size, const String& new_string);

BEE_API String& replace_range(String* src, i32 index, i32 size, const StringView& new_string);

/**
 * `substring` - Returns a string containing the characters from `src` string in the range `index` to `index` + `size`
 */
BEE_API StringView substring(const String& src, i32 index, i32 size);

BEE_API StringView substring(const String& src, i32 index);

BEE_API StringView substring(const StringView& src, i32 index, i32 size);

BEE_API StringView substring(const StringView& src, i32 index);
...
BEE_API String from_wchar(const wchar_t* wchar_str, Allocator* allocator = system_allocator());

BEE_API wchar_array_t to_wchar(const StringView& src, Allocator* allocator = system_allocator());

/**
 * String inspection utilities
 */
BEE_API bool is_space(const char character);

BEE_API bool is_digit(const char character);

/**
 * Conversion utilities
 */
BEE_API String to_string(const i32 value, Allocator* allocator = system_allocator());

BEE_API String to_string(const u32 value, Allocator* allocator = system_allocator());

BEE_API String to_string(const i64 value, Allocator* allocator = system_allocator());

BEE_API String to_string(const u64 value, Allocator* allocator = system_allocator());

BEE_API String to_string(const float value, Allocator* allocator = system_allocator());

BEE_API String to_string(const double value, Allocator* allocator = system_allocator());

/**
 * `trim` functions - for removing all occurrences of a character at the start/end of a string
 */
BEE_API String& trim_start(String* src, char character);

BEE_API String& trim_end(String* src, char character);

BEE_API String& trim(String* src, char character);
