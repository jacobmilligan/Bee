...
struct BEE_REFLECT(serializable) quaternion
{
    ...
    BEE_FORCE_INLINE quaternion() noexcept
        : w(1.0f), x(0.0f), y(0.0f), z(0.0f)
    {}

    BEE_FORCE_INLINE quaternion(const float in_w, const float in_x,
                                const float in_y, const float in_z) noexcept
        : w(in_w), x(in_x), y(in_y), z(in_z)
    {}

    explicit BEE_FORCE_INLINE quaternion(const float4& vec) noexcept
        : w(vec.w), x(vec.x), y(vec.y), z(vec.z)
    {}

    explicit BEE_FORCE_INLINE quaternion(const float4x4& mat) noexcept
    {
