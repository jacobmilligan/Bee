...
struct BEE_REFLECT(serializable) float4 : public vec<float, 4> {
    ...
    BEE_FORCE_INLINE float4() noexcept
    : x(0.0f), y(0.0f), z(0.0f), w(0.0f)
    {}

    BEE_FORCE_INLINE explicit float4(const value_t value) noexcept
    : x(value), y(value), z(value), w(value)
    {}

    BEE_FORCE_INLINE float4(const value_t cx, const value_t cy, const value_t cz, const value_t cw) noexcept
    : x(cx), y(cy), z(cz), w(cw)
    {}

    BEE_FORCE_INLINE explicit float4(const float2& cvec) noexcept
        : x(cvec.x), y(cvec.y), z(0.0f), w(0.0f)
    {}

    BEE_FORCE_INLINE float4(const float2& cvec, const value_t cz, const value_t cw) noexcept
    : x(cvec.x), y(cvec.y), z(cz), w(cw)
    {}

    BEE_FORCE_INLINE explicit float4(const float3& cvec) noexcept
        : x(cvec.x), y(cvec.y), z(cvec.z), w(0.0f)
    {}

    BEE_FORCE_INLINE float4(const float3& cvec, const value_t cw) noexcept
