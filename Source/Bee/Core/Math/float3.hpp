...
struct BEE_REFLECT(serializable) float3 : public vec<float, 3> {
    ...
    BEE_FORCE_INLINE float3() noexcept
        : x(0.0f), y(0.0f), z(0.0f)
    {}

    BEE_FORCE_INLINE explicit float3(const value_t value) noexcept
        : x(value), y(value), z(value)
    {}

    BEE_FORCE_INLINE float3(const value_t cx, const value_t cy, const value_t cz) noexcept
        : x(cx), y(cy), z(cz)
    {}

    BEE_FORCE_INLINE explicit float3(const float2& cvec) noexcept
        : x(cvec.x), y(cvec.y), z(0.0f)
    {}

    BEE_FORCE_INLINE float3(const float2& cvec, const value_t cz) noexcept
