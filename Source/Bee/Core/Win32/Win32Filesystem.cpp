...
bool last_modified(const Path& path, Timestamp* timestamp)
{
    ...
    if (FileTimeToSystemTime(&data.ftLastWriteTime, &system_time) == 0)
    {
        log_error("Failed to get last modified time: %s", win32_get_last_error_string());
        return false;
    }

    timestamp->year = system_time.wYear;
    timestamp->month = system_time.wMonth;
    timestamp->day = system_time.wDay;
    timestamp->hour = system_time.wHour;
    timestamp->minute = system_time.wMinute;
    timestamp->second = system_time.wSecond;
    timestamp->millisecond = system_time.wMilliseconds;
    return true;
}
...
bool mkdir(const Path& directory_path, const bool recursive)
{
    if (recursive)
    {
        const auto parent = directory_path.parent(temp_allocator());
        if (!parent.exists())
        {
