...
struct Win32DbgHelp
{
    ...
    ~Win32DbgHelp()
    {
        ...
        {
            log_error("Win32Debug: Failed to cleanup symbol resources: %s", win32_get_last_error_string());
            BEE_DEBUG_BREAK();
        }
...
void symbolize_stack_trace(DebugSymbol* dst_symbols, const StackTrace& trace, const i32 frame_count)
{
    ...
    for (int f = 0; f < frames_to_symbolize; ++f)
    {
        ...
        {
            log_error("StackTrace: Failed to get function name for address %p: %s", trace.frames[f], win32_get_last_error_string());
        }
    }
}
