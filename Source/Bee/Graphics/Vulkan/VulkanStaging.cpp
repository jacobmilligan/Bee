 ...
 ******************************************
 */
i32 VulkanStaging::submit_nolock(const i32 frame)
{
    ...
    transfer_queue->submit_threadsafe(1, &submit_info, buffer.submit_fence);

    buffer.is_submitted = true;
    return (frame + 1) % BEE_GPU_MAX_FRAMES_IN_FLIGHT;
}

void VulkanStaging::wait_nolock(const i32 frame)
{
    ...
    BEE_VK_CHECK(vkWaitForFences(device, 1, &buffer.submit_fence, VK_TRUE, limits::max<u64>()));
...
void VulkanStaging::init(VkDevice new_device, VulkanQueue* new_transfer_queue, VmaAllocator new_vma_allocator)
{
    ...
    BEE_VK_CHECK(vkCreateCommandPool(device, &pool_info, nullptr, &command_pool));
    ...
    BEE_VK_CHECK(vkAllocateCommandBuffers(device, &cmd_info, cmd_buffers));
    ...
    for (int frame = 0; frame < BEE_GPU_MAX_FRAMES_IN_FLIGHT; ++frame)
    {
        ...
        BEE_VK_CHECK(vkCreateFence(device, &fence_info, nullptr, &buffer.submit_fence));
        BEE_VK_CHECK(vkCreateSemaphore(device, &sem_info, nullptr, &buffer.transfer_semaphore));
    }
}
...
void VulkanStaging::destroy()
{
    ...
    for (auto& buffer : buffers)
    {
        ...
        vkDestroySemaphore(device, buffer.transfer_semaphore, nullptr);
    }

    BEE_VK_CHECK(vkResetCommandPool(device, command_pool, 0));
    vkDestroyCommandPool(device, command_pool, nullptr);
}

void VulkanStaging::ensure_capacity_nolock(const size_t capacity)
{
...
void VulkanStaging::allocate(const size_t size, const size_t alignment, VulkanStagingChunk* chunk)
{
    ...
    {
        current_frame = submit_nolock(current_frame);
        wait_nolock(current_frame); // wait for the new staging buffer to finish
