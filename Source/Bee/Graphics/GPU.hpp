...
#ifndef BEE_GPU_SUBMIT_QUEUE_SIZE
    #define BEE_GPU_SUBMIT_QUEUE_SIZE 16u
#endif // BEBEE_GPU_SUBMIT_QUEUE_SIZE
...
BEE_VERSIONED_HANDLE_32(SwapchainHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(TextureHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(TextureViewHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(BufferHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(BufferViewHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(RenderPassHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(ShaderHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(PipelineStateHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(CommandPoolHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(CommandStreamHandle) BEE_REFLECT();
BEE_VERSIONED_HANDLE_32(FenceHandle) BEE_REFLECT();
...
static constexpr i32 gpu_shader_stage_count = underlying_t(ShaderStageIndex::count);
...
struct SubmitInfo
{
    ...
    const CommandBuffer* const*     command_buffers {nullptr };
};
...
BEE_RUNTIME_API ShaderHandle gpu_create_shader(const DeviceHandle& device_handle, const ShaderCreateInfo& info);

BEE_RUNTIME_API void gpu_destroy_shader(const DeviceHandle& device_handle, const ShaderHandle& shader_handle);
...
BEE_RUNTIME_API void gpu_update_buffer(const DeviceHandle& device_handle, const BufferHandle& buffer_handle, const void* data, const size_t offset, const size_t size);
...
public:
    ...
    void bind_pipeline_state(const PipelineStateHandle& pipeline_handle);

    void bind_vertex_buffer(const BufferHandle& buffer_handle, const u32 binding, const u64 offset);
    ...
    void bind_index_buffer(const BufferHandle& buffer_handle, const u64 offset, const IndexFormat index_format);
