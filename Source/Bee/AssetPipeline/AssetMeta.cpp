...
void asset_meta_serialize(const SerializerMode& mode, const Path& location, AssetMeta* meta, AssetCompileSettings* settings, Allocator* allocator)
{
    ...
    else
    {
        ...
        auto has_type_member = type_member != doc.MemberEnd();
        if (BEE_FAIL_F(type_member == doc.MemberEnd() || type_member->value.IsString(), "Invalid settings JSON - object member `%s` is missing or not a string type", type_member_name))
        {
            return;
        }
