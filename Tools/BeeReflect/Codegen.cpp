...
void CodeGenerator::write_header_comment(const PathView& source_location)
{
 ...
 *  Source: %)" BEE_PRIsv R"(
 */
)", time_string, BEE_FMT_SV(source_location));
}
...
i32 generate_reflection(const PathView& dst_path, const ReflectedFile& file, String* output, CodegenMode mode)
{
    ...
    const auto relative_location = file.location.relative_to(dst_path.parent(), temp_allocator()).make_generic();
...
i32 generate_reflection_header(const PathView& dst_path, const ReflectedFile& file, const i32 first_type_index, String* output, CodegenMode mode)
{
    io::StringStream stream(output);
    CodeGenerator codegen(mode, &stream);
    const auto target_path = dst_path.parent();
...
void generate_typelist(const PathView& target_dir, const Span<const TypeListEntry>& all_types, CodegenMode mode, const Span<const PathView>& written_files)
{
    String output;
    io::StringStream stream(&output);
    const auto output_path = Path(target_dir, temp_allocator()).append("TypeList.init.cpp");
    ...
    fs::write_all(output_path.view(), output.view());
}
